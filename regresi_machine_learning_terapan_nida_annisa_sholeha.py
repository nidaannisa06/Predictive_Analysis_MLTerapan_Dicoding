...
- Tujuan utama dari Tahap Data Understanding dalam proyek analisis emisi CO2 kendaraan ini adalah untuk membangun pemahaman yang komprehensif mengenai karakteristik dataset, mengidentifikasi faktor-faktor yang berpotensi memengaruhi emisi CO2, dan mengeksplorasi hubungan antar variabel.
- Melalui visualisasi dan analisis deskriptif, tahap ini bertujuan untuk mengungkap pola-pola penting, mengidentifikasi potensi masalah kualitas data, dan memberikan wawasan awal yang akan mengarahkan langkah-langkah selanjutnya dalam pemrosesan data, pemilihan fitur, dan pengembangan model prediksi emisi CO2 yang efektif.
...
"""Penjelasan: Model Linear Regression menunjukkan bahwa tipe bahan bakar (fuel_type_E, D, Z, X) dan merek mobil (make_BUGATTI) adalah faktor-faktor utama yang secara linear memengaruhi emisi CO2. Perbedaan jenis bahan bakar secara signifikan berkorelasi dengan perbedaan emisi, dan merek mobil tertentu juga menunjukkan karakteristik emisi yang berbeda dalam model linear ini. Fitur-fitur terkait konsumsi bahan bakar, transmisi, dan kelas kendaraan memiliki pengaruh linear yang lebih kecil di antara 10 fitur teratas."""

# Model 2: Decision Tree Regressor
tree_model_for_importance = tree_model_drop
...
plt.title(f'Top {n_top_features} Feature Importance (Decision Tree Regressor)')
...
"""Penjelasan : Model Decision Tree Regressor sangat bergantung pada fitur konsumsi bahan bakar kombinasi (fuel_consumption_comb (mpg)). Fitur konsumsi bahan bakar kombinasi dalam satuan lain juga dianggap penting, tetapi jauh lebih sedikit. Fitur-fitur lainnya, termasuk tipe bahan bakar, konsumsi bahan bakar di kota dan jalan bebas hambatan, jenis transmisi, jumlah silinder, ukuran mesin, kelas kendaraan, dan merek mobil, memiliki pengaruh yang relatif kecil dalam prediksi emisi CO2 menurut model pohon keputusan ini (di antara 10 fitur teratas)."""
...
"""Model Random Forest Regressor juga sangat menekankan pada fitur konsumsi bahan bakar kombinasi (fuel_consumption_comb (mpg)). Fitur konsumsi bahan bakar kombinasi dalam satuan lain dan tipe bahan bakar E juga dianggap penting, tetapi dengan tingkat kepentingan yang jauh lebih rendah. Fitur-fitur lainnya dalam 10 teratas memiliki pengaruh yang relatif kecil dalam prediksi emisi CO2 menurut model ini.

Adanya kesamaan fitur terpenting antara model Decision Tree dan Random Forest (keduanya sangat menekankan pada fuel_consumption_comb (mpg)) mungkin disebabkan oleh sifat dasar kedua algoritma yang berbasis pohon.
"""
...
"""Model Gradient Boosting Regressor sangat menekankan pada fitur fuel_consumption_comb. Tipe bahan bakar E dan konsumsi bahan bakar di kota juga dianggap memiliki pengaruh yang signifikan, meskipun jauh lebih kecil dibandingkan konsumsi bahan bakar kombinasi. Fitur-fitur lainnya dalam 10 teratas memiliki pengaruh yang relatif kecil dalam prediksi emisi CO2 menurut model ini.

Perbandingan dengan model sebelumnya menunjukkan bahwa meskipun fuel_consumption_comb secara konsisten menjadi fitur terpenting, urutan dan kepentingan fitur-fitur lainnya dapat bervariasi antar model, mencerminkan bagaimana setiap algoritma secara unik menangkap pola dalam data. Gradient Boosting tampaknya memberikan bobot yang lebih tinggi pada tipe bahan bakar dan konsumsi bahan bakar berdasarkan lokasi (kota vs. jalan bebas hambatan) dibandingkan dengan model pohon tunggal dan Random Forest setelah fitur konsumsi bahan bakar kombinasi yang paling dominan.
